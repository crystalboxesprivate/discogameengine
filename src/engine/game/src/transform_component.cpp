...
vec3 TransformComponent::get_mesh_orientation_euler_angles(bool is_degrees) {
...
vec3 TransformComponent::get_forward(vec3 forward_model_space) {
  const vec4 forward_vec4 = vec4(0.f, 0.f, 1.f, 1.f);
  const mat4 orientation_matrix = mat4(orient);
