...
void EnvironmentMapFactory::load_asset_data(asset::Asset &asset) {
  EnvironmentMap &texcube = *static_cast<EnvironmentMap *>(&asset);
  String json_path = texcube.source_filename;

  Archive archive(texcube.source_filename);
  archive << texcube.texture_data;

  assert(texcube.texture_data.color_size);
  ...
  //     auto &color = document["irradiance"];
