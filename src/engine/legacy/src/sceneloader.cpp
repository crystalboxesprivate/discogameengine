...
bool load_json_scene(const char *filename) {
  ...
  if (document.HasMember(CAMERA_KEY)) {
    Guid entity = component::allocate_entity();
    auto &camera = component::add<CameraComponent>(entity);
    auto &transform = component::add<TransformComponent>(entity);
  ...
  if (document.HasMember(GAMEOBJECTS_KEY)) {
    ...
    for (u32 x = 0; x < go_js.Size(); x++) {
      ...
      auto &meta = component::add<MetadataComponent>(entity);
      ...
      auto &material = component::add<MaterialParameterComponent>(entity);
      ...
      if (go.HasMember("Mesh")) {
        using namespace runtime;
        using namespace asset;
        auto &static_mesh_component = component::add<StaticMeshComponent>(entity);
      ...
      if (go.HasMember("RigidBody")) {
        ...
        using namespace game;
        auto &comp = component::add<RigidBodyComponent>(entity);
        ...
        comp.transform = component::get_shared<TransformComponent>(entity);
      }
    }
  }
  ...
  if (document.HasMember(LIGHTS_KEY)) {
    ...
    for (u32 x = 0; x < go_js.Size(); x++) {
      ...
      auto &light_component = component::add<LightComponent>(entity);
