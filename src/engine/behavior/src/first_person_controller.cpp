...
void init_me() {
  ...
  auto &comps = component::get_components_of_type<CameraComponent>();
  // assert(comps.size());
  if (!comps.size())
    return;
  current = &component::get_ref<CameraComponent>(comps[0]);
  transform = component::get_shared<TransformComponent>(current->get_owner_id());
}
...
void FirstPersonCamera::update(double delta_time) {
  ...
  transform.get()->position = position;
}
