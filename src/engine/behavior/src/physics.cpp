...
void PhysicsBehavior::start() {
  ...
  for (auto &rb : rigid_bodies) {
    auto xfrom_ptr = component::get<game::TransformComponent>(rb.transform);
    if (!xfrom_ptr) {
      u32 entity_id = component::get_entity_id<game::RigidBodyComponent>(x);
      rb.transform = component::find<game::TransformComponent>(entity_id);
      xfrom_ptr = component::get<game::TransformComponent>(rb.transform);
    }
