...
void Renderer::draw_image() {
  ...
  auto &cameras = component::get_components_of_type<CameraComponent>();
  assert(cameras.size());
  auto &camera = component::get_ref<CameraComponent>(cameras[0]);
  ...
  auto &static_meshes = component::get_components_of_type<StaticMeshComponent>();
  for (auto &mesh_ref : static_meshes) {

    auto &mesh = component::get_ref<StaticMeshComponent>(mesh_ref);

    auto meta = component::get<game::MetadataComponent>(mesh.get_owner_id());
    ...
    ComponentRef transform = mesh.cached_transform_component.get_ref();
    if (!transform) {
      // cache transform
      mesh.cached_transform_component = component::get_shared<TransformComponent>(mesh.get_owner_id());
      transform = mesh.cached_transform_component.get_ref();
    }
    ...
    if (transform) {
      ...
      auto &transform_cast = component::get_ref<TransformComponent>(transform);
